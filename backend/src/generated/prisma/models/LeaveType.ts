...
export interface LeaveTypeDelegate<ExtArgs extends runtime.Types.Extensions.InternalArgs = runtime.Types.Extensions.DefaultArgs, GlobalOmitOptions = {}> {
   ...
   * })
   * 
   * // Create many LeaveTypes and only return the `leaveTypeId`
   * const leaveTypeWithLeaveTypeIdOnly = await prisma.leaveType.createManyAndReturn({
   ...
   * })
   * 
   * // Update zero or more LeaveTypes and only return the `leaveTypeId`
   * const leaveTypeWithLeaveTypeIdOnly = await prisma.leaveType.updateManyAndReturn({
   *   select: { leaveTypeId: true },
   *   where: {
   *     // ... provide filter here
   *   },
