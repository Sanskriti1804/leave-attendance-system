...
export interface HolidayDelegate<ExtArgs extends runtime.Types.Extensions.InternalArgs = runtime.Types.Extensions.DefaultArgs, GlobalOmitOptions = {}> {
   ...
   * 
   * // Only select the `holidayId`
   * const holidayWithHolidayIdOnly = await prisma.holiday.findMany({ select: { holidayId: true } })
   * 
   */
   ...
   * })
   * 
   * // Create many Holidays and only return the `holidayId`
   * const holidayWithHolidayIdOnly = await prisma.holiday.createManyAndReturn({
   ...
   * })
   * 
   * // Update zero or more Holidays and only return the `holidayId`
   * const holidayWithHolidayIdOnly = await prisma.holiday.updateManyAndReturn({
   *   select: { holidayId: true },
   *   where: {
   *     // ... provide filter here
   *   },
