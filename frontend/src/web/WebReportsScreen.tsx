        ...
        <WebCard>
          ...
          <View style={styles.row}>
            {(["csv", "xlsx", "pdf"] as const).map((format) => (
              ...
              >
                <Text style={styles.link}>{format.toUpperCase()}</Text>
              </TouchableOpacity>
            ))}
          </View>
        </WebCard>
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 16, fontWeight: "700", color: colors.onSurface, marginBottom: 8 },
  copy: { fontSize: 13, color: colors.secondary, lineHeight: 18, marginTop: 6 },
  ...
  row: { flexDirection: "row", gap: 8, flexWrap: "wrap", marginTop: 12, alignItems: "center" },
  input: {
    minWidth: 140,
    minHeight: 44,
    borderWidth: 1,
  ...
  primary: { backgroundColor: colors.accent, borderRadius: 8, paddingVertical: 12, paddingHorizontal: 16 },
  primaryText: { color: colors.onPrimary, fontWeight: "700" },
  tr: { flexDirection: "row" },
  th: { fontWeight: "700" },
  cell: { width: 140, paddingVertical: 6, paddingRight: 8, fontSize: 12, color: colors.onSurface },
  link: { fontWeight: "700", color: colors.accentDeep, marginRight: 12 },
});
