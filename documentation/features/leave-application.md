...
Confirmed LV-APP-01–17, duration LV-DUR, dates LV-DATE-01–02/06–07, overlap LV-OVR-01. **Leave applications HTTP API implemented** (zigzag selections, status-aware overlap, 14-day advance). Leave type CRUD is implemented in the leave-types module. Medical document upload is a separate module and is integrated on submit.
...
Leave type CRUD (separate module). Medical document upload (separate module). Leave balance checks. Sandwich-filling weekends/holidays (CAL-07).
...
2026-09-18 — Apply Leave defaults to Casual; operational notification checkbox; success toast after submit/draft. Review primary actions are Approve and Reject; HR Note is secondary.
