...
- Admin / guest_admin: Dashboard `/admin`, Leave review `/leave/admin-review`, People `/people`, Reports `/reports` (live `/api/v1/reports/{slug}`), More `/settings`. From More: `/admin-profile` (compact audit for admin), `/org-settings` (GET live; PATCH admin-only).
...
On **web** (`expo start --web`), the same routes render a desktop UI under `frontend/src/web/` using the same cream/teal tokens as Android (floating ink nav rail, teal active icon well, Inter, left-accent cards). Under 960px the rail becomes a horizontal pill. Mobile screen files under `frontend/src/modules/` are unchanged. Shared services and `/api/v1` calls are reused. Employee Profile cards stack full width with initials when no photo. Apply Leave is one scrollable card with a month calendar (from–to range, same session payload) and a medical upload surface. Employee Home clock/attendance is full canvas width; remaining dashboard cards are equal-width. Debug “API DATA” labels are not shown. Admin web: equal-size HR action grid; leave review as aligned rows with Android-styled action buttons and a centered HR-note dialog; Reports grouped leave/attendance chips without a zero KPI strip; Admin Profile matches Employee Profile stacked cards, with Notifications and Audits as visible cards.
...
Ink primary `#1A1A1A`, cream canvas `#F3EEE6`, teal accent `#0F766E`, session blue `#2563EB`. Page gradient blends sand, cream, teal wash, and mist. Shared chrome: ink/teal bottom nav (Android) and matching ink/teal web rail, editorial kickers, restrained radii (8–10px), left-accent surfaces. Attendance punch logic is unchanged; Home clock/punch chrome is visual only.
