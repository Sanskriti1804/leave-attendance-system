...
Leave list and apply keep live `/api/v1` leave calls. Admin review approve/reject is admin-only; guest_admin is read-only. Medical document bytes are not fetched for employees (MED-09). Apply Leave uploads via `POST /api/v1/documents` after the file picker. Home punch buttons remain local simulation; reports APIs are not implemented.
