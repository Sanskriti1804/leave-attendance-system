...
On **web** (`expo start --web`), the same routes render a desktop UI under `frontend/src/web/` using the same cream/teal tokens as Android (floating ink nav rail, teal active icon well, Inter, left-accent cards). Under 960px the rail becomes a horizontal pill. Mobile screen files under `frontend/src/modules/` are unchanged. Shared services and `/api/v1` calls are reused. Employee Profile cards stack full width. Apply Leave is one scrollable card with a month calendar (from–to range, same session payload) and a medical upload surface. Employee Home clock/attendance is full canvas width; remaining dashboard cards are narrower. Debug “API DATA” labels are not shown. Admin web: equal-size HR action grid; leave review as aligned rows with button actions and a centered HR-note dialog; Reports grouped leave/attendance chips without a zero KPI strip; Admin Profile is a full-width card with Notifications and Audits opened from navigation tiles.
